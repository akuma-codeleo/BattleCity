...
std::string ResourceManager::getFileString(const std::string& relativFilePath)
{
	std::ifstream f;
	f.open(m_path + "/" + relativFilePath.c_str(), std::ios::in | std::ios::binary);
	if (!f.is_open())
	{
		std::cerr << "Failed to open file: " << relativFilePath << std::endl;
		return std::string{};
	}
...
bool ResourceManager::loadJSONResources(const std::string& JSONPath)
{
	...
	if (animatedSpritesIt != document.MemberEnd())
	{
		for (const auto& currentAnimatedSprite : animatedSpritesIt->value.GetArray())
		{
			...
			auto pAnimatedSprite = loadAnimatedSprite(name, textureAtlas, shader, width, height, initialSubTexture);
			if (!pAnimatedSprite)
			{
				continue;
			}
