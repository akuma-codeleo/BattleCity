...
Level::Level(const std::vector<std::string>& levelDescription)
{
	...
	{
		unsigned int currentLeftOfsset = 0;
		for (const char currentElement : currentRow)
		{
			m_mapObjects.emplace_back(createGameObjectFromDescription(currentElement, glm::vec2(currentLeftOfsset, currentBottomOffset), glm::vec2(BLOCK_SIZE, BLOCK_SIZE), 0.f));

			currentLeftOfsset += BLOCK_SIZE;
		}
